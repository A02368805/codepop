   ...
   return (
      <View style={styles.paymentStatus}>
        <View style={[styles.statusIcon, { backgroundColor: STATUS_CONTENT_MAP[status].iconColor }]}>
          <Text>{STATUS_CONTENT_MAP[status].icon}</Text>
        </View>
        ...
        {intentId && (
          ...
          >
            ...
            {/* SVGs are not natively supported in React Native, but you can use an image, icon, or a library like react-native-svg */}
            {/* This example skips SVG; add it back if needed with react-native-svg */}
            </TouchableOpacity>
            )}

        <TouchableOpacity onPress={() => Linking.openURL('/checkout')} style={styles.retryButton}>
