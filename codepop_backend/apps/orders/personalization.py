...
from .catalog import ADD_IN_OPTIONS, ICE_CREAM_OPTIONS, MENU_ITEMS, SODA_OPTIONS, SYRUP_OPTIONS
...
def build_user_taste_profile(user):
    ...
    for preference in preferences:
        ...
        if preference.preference_type == TastePreference.PreferenceType.FAVORITE_SODA:
            profile["favorite_sodas"].add(token)
        elif preference.preference_type == TastePreference.PreferenceType.FAVORITE_SYRUP:
            profile["favorite_syrups"].add(token)
        elif preference.preference_type == TastePreference.PreferenceType.FAVORITE_ADD_IN:
            profile["favorite_add_ins"].add(token)
        elif preference.preference_type == TastePreference.PreferenceType.FAVORITE_ICE_CREAM:
            profile["favorite_ice_creams"].add(token)
...
def _option_allowed(option, dietary_preferences):
    ...
    if "caffeine-free" in dietary_preferences and "caffeinated" in tags:
        return False
    if "zero-sugar" in dietary_preferences and "zero-sugar" not in tags and "diet" not in tags:
        return False
...
def _pick_best(tokens, option_map, preferred_tokens, counter, dietary_preferences, fallback=None):
    candidates = []
...
def _preferred_size(profile, current_size):
    ...
    if profile["adventurousness_preference"] == User.AdventurousnessPreference.ADVENTUROUS:
        return "large"
    return "medium"
...
def _recommended_syrups(profile, current_syrups, menu_item):
    ...
    preferred_rank.extend(
        token for token in profile["favorite_syrups"] if token not in preferred_rank and token not in profile["dislikes"]
    )
    preferred_rank.extend(
        token for token in menu_item["default_syrups"] if token not in preferred_rank and token not in profile["dislikes"]
    )

    target_count = 2 if profile["adventurousness_preference"] == User.AdventurousnessPreference.ADVENTUROUS else 1
...
def _recommended_add_ins(profile, current_add_ins, menu_item, chosen_soda):
    ...
    candidates.extend(
        token for token in profile["favorite_add_ins"] if token not in candidates and token not in profile["dislikes"]
    )
    candidates.extend(
        token for token in menu_item["default_add_ins"] if token not in candidates and token not in profile["dislikes"]
    )
...
def _recommended_ice_cream(profile, current_ice_cream, menu_item):
    ...
    if menu_item.get("default_ice_cream") and (
        ...
        or profile["adventurousness_preference"] == User.AdventurousnessPreference.ADVENTUROUS
    ):
...
def recommend_builder_configuration(*, user, menu_item, current_selection=None):
    ...
    if profile["favorite_sodas"] and soda in profile["favorite_sodas"]:
        reasons.append(f"Picked {SODA_OPTIONS[soda]['label']} because it matches your saved favorite sodas.")
    elif profile["soda_counter"][soda]:
        reasons.append(f"Picked {SODA_OPTIONS[soda]['label']} because you have ordered similar bases before.")
    else:
        reasons.append(f"Started with {SODA_OPTIONS[soda]['label']} because it fits this signature drink well.")
...
def recommend_drink_menu_scores(user):
    ...
    for slug, item in MENU_ITEMS.items():
        ...
        if item.get("default_ice_cream") and item["default_ice_cream"] in profile["favorite_ice_creams"]:
            score += 2
            reasons.append("Its float topper matches your saved ice cream picks.")

        item_tokens = set(item["default_syrups"]) | set(item["default_add_ins"]) | {item["default_soda"], item.get("default_ice_cream", "")}
