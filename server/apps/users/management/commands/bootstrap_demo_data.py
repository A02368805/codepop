...
STORE_DEFINITIONS = {
    "A": [
        ("A001", "Chicago Loop", "Chicago", "IL", "120 W Lake St", "60601", "41.885311", "-87.631779"),
        ("A002", "Evanston North", "Evanston", "IL", "807 Church St", "60201", "42.048213", "-87.683303"),
    ],
    "B": [
        ("B001", "Jersey City Exchange", "Jersey City", "NJ", "30 Hudson St", "07302", "40.719446", "-74.034166"),
        ("B002", "Newark Broad", "Newark", "NJ", "744 Broad St", "07102", "40.735657", "-74.172366"),
    ],
    "C": [
        ("C001", "Logan Main", "Logan", "UT", "123 Main St", "84321", "41.736980", "-111.833836"),
        ("C002", "North Logan Canyon", "North Logan", "UT", "456 Canyon Rd", "84341", "41.769089", "-111.804093"),
        ("C003", "Providence Riverwoods", "Providence", "UT", "112 Riverwoods Pkwy", "84332", "41.704611", "-111.817970"),
        ("C004", "Smithfield Center", "Smithfield", "UT", "77 Center St", "84335", "41.838265", "-111.832178"),
        ("C005", "Brigham City Forest", "Brigham City", "UT", "15 Forest St", "84302", "41.510217", "-112.015502"),
        ("C006", "Ogden Riverdale", "Ogden", "UT", "401 24th St", "84401", "41.223000", "-111.973830"),
        ("C007", "Layton East", "Layton", "UT", "520 Hill Field Rd", "84041", "41.060221", "-111.971053"),
        ("C008", "Bountiful South", "Bountiful", "UT", "75 Orchard Dr", "84010", "40.889389", "-111.880771"),
        ("C009", "Salt Lake Downtown", "Salt Lake City", "UT", "210 State St", "84111", "40.762761", "-111.887985"),
        ("C010", "Murray Central", "Murray", "UT", "95 Vine St", "84107", "40.658565", "-111.888274"),
        ("C011", "Sandy Alta", "Sandy", "UT", "150 10600 S", "84070", "40.558908", "-111.890801"),
        ("C012", "Draper Peaks", "Draper", "UT", "390 12300 S", "84020", "40.526133", "-111.863823"),
        ("C013", "Lehi Tech", "Lehi", "UT", "260 Aspen Ave", "84043", "40.391617", "-111.850766"),
        ("C014", "American Fork Main", "American Fork", "UT", "90 Main St", "84003", "40.376897", "-111.795764"),
        ("C015", "Orem University", "Orem", "UT", "760 State St", "84058", "40.296898", "-111.694648"),
        ("C016", "Provo Center", "Provo", "UT", "42 University Ave", "84601", "40.233844", "-111.658534"),
        ("C017", "Spanish Fork Canyon", "Spanish Fork", "UT", "150 800 N", "84660", "40.114955", "-111.654923"),
        ("C018", "Heber Valley", "Heber City", "UT", "345 Main St", "84032", "40.507008", "-111.413798"),
        ("C019", "Park City Summit", "Park City", "UT", "675 Park Ave", "84060", "40.646063", "-111.498014"),
        ("C020", "Tooele Gateway", "Tooele", "UT", "980 Main St", "84074", "40.530777", "-112.298279"),
    ],
    "D": [
        ("D001", "Dallas North", "Dallas", "TX", "410 Elm St", "75201", "32.781609", "-96.800473"),
        ("D002", "Plano Legacy", "Plano", "TX", "7600 Windrose Ave", "75024", "33.077904", "-96.821254"),
    ],
    "E": [
        ("E001", "Atlanta Midtown", "Atlanta", "GA", "999 Peachtree St", "30309", "33.781334", "-84.383108"),
        ("E002", "Marietta Square", "Marietta", "GA", "11 North Park Sq", "30060", "33.952602", "-84.549932"),
    ],
    "F": [
        ("F001", "Phoenix Central", "Phoenix", "AZ", "240 Camelback Rd", "85012", "33.509540", "-112.070427"),
        ("F002", "Mesa Gateway", "Mesa", "AZ", "180 Main St", "85201", "33.415184", "-111.831473"),
        ("F003", "Tempe Lakes", "Tempe", "AZ", "99 Mill Ave", "85281", "33.430569", "-111.939627"),
        ("F004", "Chandler Square", "Chandler", "AZ", "140 Arizona Ave", "85225", "33.303202", "-111.841250"),
        ("F005", "Gilbert Ranch", "Gilbert", "AZ", "75 Guadalupe Rd", "85233", "33.362821", "-111.789028"),
    ],
    "G": [
        ("G001", "Boise Capitol", "Boise", "ID", "50 Idaho St", "83702", "43.615018", "-116.202313"),
        ("G002", "Meridian Village", "Meridian", "ID", "110 Main St", "83642", "43.612109", "-116.391513"),
        ("G003", "Nampa Market", "Nampa", "ID", "215 12th Ave", "83651", "43.540718", "-116.563462"),
        ("G004", "Caldwell Riverfront", "Caldwell", "ID", "88 Dearborn St", "83605", "43.662938", "-116.687359"),
        ("G005", "Eagle Foothills", "Eagle", "ID", "170 State St", "83616", "43.695442", "-116.354434"),
    ],
}
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self._seed_suppliers(stores=stores, regions=regions, inventory_items=inventory_items, users=users)
        ...
        self._seed_transfers(stores=stores, hubs=hubs, inventory_items=inventory_items, users=users)
    ...
    def _reset_demo_data(self):
        ...
        User.objects.filter(
            email__iendswith="@floatstack.local"
        ).delete()
        User.objects.filter(
            email__iendswith="@codepop.local"
        ).delete()
    ...
    def _seed_stores(self, regions):
        ...
        for region_code, rows in STORE_DEFINITIONS.items():
            for store_code, name, city, state_code, address, postal_code, latitude, longitude in rows:
    ...
    def _seed_inventory_catalog(self):
        inventory_items = {}
        for sku, name, category, unit_of_measure, threshold, is_perishable, requires_frozen_storage in catalog_inventory_definitions():
    ...
    def _seed_users(self, *, password, regions, stores):
        ...
        def upsert_user(email, *, role, first_name, last_name, default_region=None, preferred_store=None, is_superuser=False):
        ...
        for region_code, region in regions.items():
            logistics_manager = users[f"logistics.{region_code.lower()}@floatstack.local"]
    ...
    def _seed_balances(self, *, stores, hubs, inventory_items):
        ...
        for index, store in enumerate(Store.objects.order_by("store_code"), start=1):
            for item_index, item in enumerate(ordered_items, start=1):
                ...
                if store.region.code == "C" and index % 5 == 0 and item.sku in {"SYRUP-STRAWBERRY", "CUPS-24OZ"}:
        ...
        for hub_index, hub in enumerate(SupplyHub.objects.order_by("hub_code"), start=1):
            for item_index, item in enumerate(ordered_items, start=1):
                balance = get_hub_balance(hub, item)
                balance.on_hand_quantity = item.default_low_stock_threshold * Decimal(str(12 + hub_index + item_index))
                balance.reserved_quantity = Decimal("0.00")
                balance.save()

    def _seed_suppliers(self, *, stores, regions, inventory_items, users):
        supplier_rows = [
            ("Cache Valley Dairy", "C", "Morgan Supplier", "cache-dairy@floatstack.local", "801-555-0110"),
            ("Treasure Valley Beverage", "G", "Parker Supply", "treasure-bev@floatstack.local", "208-555-0140"),
            ("Sonoran Packaging", "F", "Avery Vendor", "sonoran-pack@floatstack.local", "602-555-0190"),
        ]
        suppliers = {}
        ...
        for region_code, store_code, sku, quantity_received, recorded_by_email in replenishment_rows:
        ...
        for region_code, store_code, sku, quantity_requested, requested_by_email in pending_order_rows:
            SupplierReplenishment.objects.update_or_create(
                supplier=suppliers[region_code],
                store=stores[store_code],
                inventory_item=inventory_items[sku],
    ...
    def _seed_machine_types(self):
        machine_types = {}
        for code, name, default_interval, warning_days, error_days in MACHINE_TYPE_DEFINITIONS:
        ...
        for store_code, machine_type_code, operational_from_date in machine_specs:
            ...
            machine, _ = Machine.objects.update_or_create(
                ...
                defaults={
                    ...
                    "next_service_due_date": operational_from_date + timedelta(days=machine_types[machine_type_code].default_service_interval_days),
                    "is_active": True,
                },
            )
    ...
    def _seed_maintenance_policies(self, *, regions, machine_types):
        ...
        for machine_type_code, region_code, max_days, warning_days, schedule_days in policy_rows:
    ...
    def _seed_orders(self, *, stores, users):
        ...
        for row in demo_order_rows:
            if Order.objects.filter(public_order_code=row["public_order_code"]).exists():
                continue

            customer = users.get(row["customer_email"]) if row["customer_email"] else None
            order = create_order(
            ...
            record_payment_success(order, payment_intent_id=row["payment_intent"], actor=customer)
            for status in row.get("post_payment_transitions", []):
                if order.status != status:
                    transition_order_status(order, status, actor=customer)
            if row.get("refund"):
                record_refund(order, actor=users["super_admin"], notes="Demo refund before preparation.")
    ...
    def _seed_transfers(self, *, stores, hubs, inventory_items, users):
        same_region_transfer = SupplyTransfer.objects.filter(notes="DEMO same-region transfer").first()
        ...
        cross_region_transfer = SupplyTransfer.objects.filter(notes="DEMO cross-region hub transfer").first()
        if cross_region_transfer is None:
            ...
            receive_transfer(cross_region_transfer, actor=users["manager.c002@floatstack.local"])
    ...
    def _run_sample_imports(self, *, users):
        ...
        if not repair_job_exists:
            import_repair_status_csv(
                self.seed_csv_dir.joinpath("maintenance_schedule_sample.csv").read_text(),
                uploaded_by=users["repair.north@floatstack.local"],
                original_filename="maintenance_schedule_sample.csv",
            )

        schedule = SupplySchedule.objects.filter(created_by_ai=True).order_by("created_at").first()
        if schedule and schedule.status == SupplySchedule.Status.DRAFT:
            approve_supply_schedule(schedule, approver=users["logistics.c@floatstack.local"])
    ...
    def _seed_notifications(self, *, users):
        notification_rows = [
            ("manager.c001@floatstack.local", "Low stock watch", "Logan Main has balances below threshold after queued demo orders."),
            ("logistics.c@floatstack.local", "Import review", "Supply usage import produced review-ready AI schedule drafts for Region C."),
            ("repair.north@floatstack.local", "Repair escalation", "A Region C machine escalated from warning to out-of-order."),
        ]
