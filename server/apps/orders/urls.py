...
urlpatterns = [
    ...
    path(
        "cart/items/<str:item_id>/update/",
        CartItemUpdateView.as_view(),
        name="cart-item-update",
    ),
    path(
        "cart/items/<str:item_id>/remove/",
        CartItemRemoveView.as_view(),
        name="cart-item-remove",
    ),
    path("checkout/", CheckoutView.as_view(), name="checkout"),
    path(
        "checkout/validate/", CheckoutValidateView.as_view(), name="checkout-validate"
    ),
    path(
        "confirmation/<str:order_code>/",
    ...
    path(
        "favorites/<uuid:favorite_id>/remove/",
        FavoriteRemoveView.as_view(),
        name="favorite-remove",
    ),
    path(
        "favorites/<uuid:favorite_id>/add-to-cart/",
        FavoriteAddToCartView.as_view(),
        name="favorite-add-to-cart",
    ),
    ...
    path(
        "<uuid:order_id>/mark-preparing/",
        MarkPreparingView.as_view(),
        name="mark-preparing",
    ),
    path("<uuid:order_id>/mark-ready/", MarkReadyView.as_view(), name="mark-ready"),
    path(
        "<uuid:order_id>/mark-picked-up/",
        MarkPickedUpView.as_view(),
        name="mark-picked-up",
    ),
]
