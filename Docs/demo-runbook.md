...
5. In `/maintenance/`, run one live assignment action (acknowledge or start) and show the assignment card/audit activity update.
6. Sign in as `logistics.c@floatstack.local` and show the logistics dashboard, `/supply-hubs/`, `/imports/`, and `/sync/` for transfers, AI draft schedules, import outcomes, receiver projections, and logged sync conflicts.
7. In `/sync/`, resolve one open conflict (or mark ignored) to demonstrate reconciliation controls.
8. Sign in as `superadmin@floatstack.local` and show the super-admin dashboard plus `/analytics/` for cross-region comparisons, daily revenue, order-backed financial rows, maintenance summaries, AI schedule visibility, sync health, and audit visibility.
9. Finish with an account or guest order flow to show the customer path still works inside the same product shell.
...
- Sync conflict actions are explicit: open conflicts can be resolved or ignored from the sync workspace panel.
